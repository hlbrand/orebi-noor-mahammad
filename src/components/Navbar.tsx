  ...
  return (
    <div className="w-full h-20 bg-white border-b-[1px] border-b-gray-400 sticky top-0 z-50">
      <nav className="h-full max-w-screen-xl mx-auto px-4 xl:px-0 flex items-center justify-between gap-2">
        ...
        <div className="hidden md:inline-flex items-center gap-2">
          {navBarList.map((item) => (
            <Link
              href={item?.link}
              key={item?.link}
              className={`flex hover:font-medium w-20 h-6 justify-center items-center px-12 text-gray-600 hover:underline underline-offset-4 decoration-[1px] hover:text-gray-950 md:border-r-[2px] md:border-gray-400 duration-200 last:border-r-0 ${
                pathname === item?.link
                  ? "font-medium underline underline-offset-4 decoration-[1px] text-gray-950"
                  : ""
              }`}
            >
              {item?.title}
        ...
        <HiMenuAlt2 className="inline-flex md:hidden cursor-pointer w-8 h-6 text-primeColor" />
      </nav>
    </div>
  );
};
